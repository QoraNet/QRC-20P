...
impl PrivacyCircuitBuilder {
    ...
    pub fn new_for_keygen(k: usize, lookup_bits: usize) -> Self {
        ...
        // ✅ Use witness_gen_only = true for keygen
        let builder = BaseCircuitBuilder::new(true).use_k(k).use_lookup_bits(lookup_bits);
        eprintln!("🔍 DEBUG: Keygen builder created, k={}, lookup_bits={:?}", k, builder.lookup_bits());
        Self {
            builder,
            lookup_bits,
            k,
    ...
    pub fn new(k: usize, lookup_bits: usize) -> Self {
        ...
        // We have 2 public outputs: commitment and nullifier
        builder.set_instance_columns(1); // 1 instance column with 2 values

        eprintln!("🔍 DEBUG: Builder created, k={}, lookup_bits={:?}", k, builder.lookup_bits());
        Self {
            builder,
            lookup_bits,
            k,
...
impl ProductionProofSystem {
    ...
    pub fn new(k: u32, lookup_bits: usize) -> Result<Self> {
        ...
        let mut single_circuit_builder = PrivacyCircuitBuilder::new(k as usize, lookup_bits);
        ...
        // ✅ Create witness-less circuit for keygen (structure only)
        let mut single_circuit_no_witness = single_circuit.without_witnesses();

        // ✅ Calculate params on the witness-less circuit (keygen uses this)
        eprintln!("🔍 DEBUG: Calling calculate_params on witness-less circuit");
        let single_config_params = single_circuit_no_witness.builder.calculate_params(Some(9));

        // Debug: show assigned_instances shape for keygen
        eprintln!("🔍 DEBUG (keygen): assigned_instances.len() = {}", single_circuit_no_witness.builder.assigned_instances.len());
        for (i, vec) in single_circuit_no_witness.builder.assigned_instances.iter().enumerate() {
            eprintln!("  phase {}: {} public instances", i, vec.len());
        }

        // ✅ Generate keys using witness-less circuit
        eprintln!("🔍 DEBUG: Generating single transfer keys");
        let single_vk = keygen_vk(&params.params, &single_circuit_no_witness.builder)?;
        let single_pk = keygen_pk(&params.params, single_vk.clone(), &single_circuit_no_witness.builder)?;

        // ✅ Extract break points from keygen circuit
        let single_break_points = single_circuit_no_witness.builder.break_points();
        eprintln!("🔍 DEBUG: Saved {} break points for single circuit", single_break_points.len());

        // Generate keys for TRANSACTIONS
        eprintln!("🔍 DEBUG: Creating transaction keygen circuit");
        let mut transaction_circuit_builder = BalanceProofCircuitBuilder::new(k as usize, lookup_bits);
        ...
        // ✅ Create witness-less circuit for keygen (structure only)
        let mut transaction_circuit_no_witness = transaction_circuit.without_witnesses();

        // ✅ Calculate params on the witness-less circuit (keygen uses this)
        let transaction_config_params = transaction_circuit_no_witness.builder.calculate_params(Some(9));

        // Debug: show assigned_instances shape for keygen
        eprintln!("🔍 DEBUG (keygen): transaction assigned_instances.len() = {}", transaction_circuit_no_witness.builder.assigned_instances.len());
        for (i, vec) in transaction_circuit_no_witness.builder.assigned_instances.iter().enumerate() {
            eprintln!("  phase {}: {} public instances", i, vec.len());
        }

        // ✅ Generate keys using witness-less circuit
        eprintln!("🔍 DEBUG: Generating transaction keys");
        let transaction_vk = keygen_vk(&params.params, &transaction_circuit_no_witness.builder)?;
        let transaction_pk = keygen_pk(&params.params, transaction_vk.clone(), &transaction_circuit_no_witness.builder)?;

        // ✅ Extract break points from keygen circuit
        let transaction_break_points = transaction_circuit_no_witness.builder.break_points();
        eprintln!("🔍 DEBUG: Saved {} break points for transaction circuit", transaction_break_points.len());
